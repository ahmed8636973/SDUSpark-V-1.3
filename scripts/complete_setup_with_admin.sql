-- Complete EDUSpark Database Setup with Admin User
-- This script creates all necessary tables and sets up the admin account
...
CREATE POLICY "Anyone can view packages" ON packages FOR SELECT TO authenticated USING (true);
CREATE POLICY "Admins can manage packages" ON packages FOR ALL USING (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
);

-- Create RLS policies for sub_packages
CREATE POLICY "Anyone can view sub_packages" ON sub_packages FOR SELECT TO authenticated USING (true);
...
BEGIN
  ...
  INSERT INTO auth.users (
    ...
    '{"full_name": "Ahmed Admin", "role": "admin"}',
    NOW(),
    NOW(),
    '',
    '',
